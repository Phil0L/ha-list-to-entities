"""Test the Todolist to entities config flow."""
...
async def test_config_flow(
    ...
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        {"name": "My list_to_entities", "entity_id": input_sensor_entity_id},
    )
    await hass.async_block_till_done()

    # assert result["type"] is FlowResultType.CREATE_ENTRY
    # assert result["title"] == "My list_to_entities"
    # assert result["data"] == {}
    ...
    assert config_entry.title == "My list_to_entities"
...
async def test_options(hass: HomeAssistant, platform) -> None:
    ...
    state = hass.states.get(f"{platform}.my_list_to_entities")
